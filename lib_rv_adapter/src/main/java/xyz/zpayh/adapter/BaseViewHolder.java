...
public class BaseViewHolder extends RecyclerView.ViewHolder {
    ...
    void setOnItemClickListener(OnItemClickListener onItemClickListener) {
        ...
        if (onItemClickListener != null && !mInitClickListener){
            ...
            this.itemView.setOnClickListener(mOnClickListener); // 这个虽然是给ItemView添加的事件，但是也是可以接受ItemView中的子View的点击事件。因为事件传递
        }
    }
     ...
     */
    public BaseViewHolder setClickable(@IdRes int id, boolean clickable){ // 如果clickable为true的话，
        View view = find(id);
        if (view != null){
            if (clickable){
    ...
    public <T extends View> T find(@IdRes int viewId){
        ...
        for (int i = 0; i < mIdsAndViews.length; i+=2) {
            ...
            if (id != null && id == viewId){ // 这里我认为应该 是id.equals(viewId)比较好
                return (T) mIdsAndViews[i+1];
            }

            if (id == null){ // 因为mIdsAndViews是Object[]类型，并且是以2的倍数式扩容，所以取出id可能为null
                indexToAdd = i;
            }
        }
        ...
        if (indexToAdd == -1){
            ...
            mIdsAndViews = Arrays.copyOf(mIdsAndViews,
                    indexToAdd < 2 ? 2 : indexToAdd * 2); // 给mIdsAndViews扩容，是2的倍数式的扩容
        }
